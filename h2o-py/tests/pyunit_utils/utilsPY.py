...
def check_sorted_2_columns(frame1, sorted_column_indices, prob=0.5, ascending=[True, True]):
    temp1 = frame1.as_data_frame(use_pandas=False)
        ...
        for rowInd in range(0, frame1.nrow-1):
            if (random.uniform(0.0,1.0) < prob):
                if colInd == sorted_column_indices[0]:
                    if not(math.isnan(temp1[rowInd][ colInd])) and not(math.isnan(temp1[rowInd+1][colInd])):
                        if ascending[colInd]:
                            assert temp1[rowInd][colInd] <= temp1[rowInd+1][colInd], "Wrong sort order: value at row {0}: {1}, value at " \
                                                               "row {2}: {3}".format(rowInd, temp1[rowInd][colInd],
                                                                                     rowInd+1, temp1[rowInd+1][colInd])
                        else:
                            assert temp1[rowInd][colInd] >= temp1[rowInd+1][colInd], "Wrong sort order: value at row {0}: {1}, value at " \
                                                                                     "row {2}: {3}".format(rowInd, temp1[rowInd][colInd],
                                                                                                           rowInd+1, temp1[rowInd+1][colInd])
                else: # for second column
                    if not(math.isnan(temp1[rowInd][sorted_column_indices[0]])) and not(math.isnan(temp1[rowInd+1][sorted_column_indices[0]])):
                        if (temp1[rowInd][sorted_column_indices[0]]==temp1[rowInd+1][sorted_column_indices[0]]):  # meaningful to compare row entries then
                            if not(math.isnan(temp1[rowInd][colInd])) and not(math.isnan(temp1[rowInd+1][colInd])):
                                if ascending[colInd]:
                                    assert temp1[rowInd][colInd] <= temp1[rowInd+1][colInd], "Wrong sort order: value at row {0}: {1}, value at " \
                                                                           "row {2}: {3}".format(rowInd, temp1[rowInd][colInd],
...
def check_sorted_1_column(frame1, sorted_column_index, prob=0.5, ascending=True):
    ...
    for rowInd in range(0, frame1.nrow-1, skipRow):
        if not (math.isnan(temp1[rowInd][sorted_column_index])) and not (
            math.isnan(temp1[rowInd + 1][sorted_column_index])):
            if ascending:
                assert temp1[rowInd][sorted_column_index] <= temp1[
                    rowInd + 1][sorted_column_index], "Wrong sort order: value at row {0}: {1}, value at " \
                                                      "row {2}: {3}".format(rowInd,
                                                                            temp1[rowInd][sorted_column_index],
                                                                            rowInd + 1,
                                                                            temp1[rowInd + 1][sorted_column_index])
            else:
                assert temp1[rowInd][sorted_column_index] >= temp1[
                    rowInd + 1][sorted_column_index], "Wrong sort order: value at row {0}: {1}, value at " \
                                                      "row {2}: {3}".format(rowInd,
